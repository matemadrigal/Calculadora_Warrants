 ...
 */
function blackScholes(params: WarrantInputs): number {
  const {
    type,
    spot,
    strike,
 ...
 */
function calculateDelta(params: WarrantInputs): number {
  const {
    type,
    spot,
    strike,
 ...
 */
function calculateVega(params: WarrantInputs): number {
  const {
    spot,
    strike,
    daysToExpiration,
 ...
 */
function calculateImpliedVol(params: WarrantInputs): number | null {
...
const WarrantCalculator: React.FC = () => {
  // Estados
  const [warrantType, setWarrantType] = useState<'call' | 'put'>('put');
  const [spot, setSpot] = useState<string>('6000');
  const [strike, setStrike] = useState<string>('6500');
  ...
  const plEvolutionData = useMemo(() => {
    if (!results || calculationMode === 'impliedVol') return [];
    ...
    const data = [];
    for (let price = minPrice; price <= maxPrice; price += step) {
      const point: any = { price: parseFloat(price.toFixed(2)) };
            ...
            <div className="bg-[#1E293B] rounded-lg border border-[#334155] p-6 sticky top-6">
              ...
              <div className="mt-6">
                ...
                <select
                  ...
                  onChange={(e) => setCalculationMode(e.target.value as 'volatility' | 'impliedVol')}
